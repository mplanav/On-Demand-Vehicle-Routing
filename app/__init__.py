...
def start_planner():
    global walls, tracks, rfids, width, height
    try:
        walls, tracks, rfids, width, height = read_map_json("mapa.json")
        return jsonify({"message": "Mapa cargado. Esperando checkpoints..."})
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route("/set-checkpoints", methods=["POST"])
def set_checkpoints():
    ...
    planner = DStarLite(start, goal, walls, tracks, width, height)
    planner.compute_shortest_path()

    path = planner.get_path()
    if not path:
        return jsonify({"error": "No hay camino disponible"}), 400
